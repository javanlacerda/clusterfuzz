  ...
  def _cross_pollinate_other_fuzzer_corpuses(self):
    ...
    for cross_pollinate_fuzzer in self.cross_pollinate_fuzzers:
      ...
      if not storage.exists(corpus_backup_url, ignore_errors=True):
        ...
        logs.warning('Corpus backup does not exist, ignoring: %s.' %
                     corpus_backup_url)
        continue
      ...
      if result:
        ...
        logs.error('Failed to unpack corpus backup from url %s.' %
                   corpus_backup_url)
...
class Runner:
  ...
  def __init__(self, build_directory, context):
    ...
    if not self.target_path:
      raise CorpusPruningError('Failed to get fuzzer path for %s.' %
...
class CorpusPruner:
  ...
  def process_bad_units(
      self, bad_units_path, quarantine_corpus_path
...
def _save_coverage_information(output):
  ...
  try:
    ...
    # impossible.
    raise CorpusPruningError('Failed to save corpus pruning result: %s.' %
                             repr(e))
